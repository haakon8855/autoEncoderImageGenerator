...
class DataMode(Enum):
    ...
    Use for testing coverage etc. Data represented by their
    float values (not binarized).
    ...
    MONO_FLOAT_MISSING:
    Standard one-channel MNIST dataset, but one class taken out.
    Use for testing anomaly detection use-case. Data represented by
    their float values (not binarized).
    Can be easier to learn, but does not give as easy a probabilistic understanding.
    """
    MONO_FLOAT_MISSING = auto()
    """
    COLOR_<WHATEVER>:
    These are *STACKED* versions of MNIST, i.e., three color channels
    with one digit in each channel.
...
class StackedMNISTData:
    """
    The class will provide examples of data by sampling uniformly from MNIST data.
    We can do this one-channel (black-and-white images) or multi-channel
    (*STACKED* data), in which the last dimension will be the "color channel"
    of the image. In this case, 3 channels is the most natural, in which
    case each channel is one color (e.g. RGB).
    ...
    def __init__(self,
        ...
        (self.train_images,
         self.train_labels), (self.test_images,
